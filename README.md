...
![LaTeX 题库组合模式热力图](docs/screenshots/heatmap.png)
...
- `assets/` 保存插入的图片。
- `exports/` 保存成功导出的文件。
- `.tmp/` 保存当前题编译和导出过程中的临时文件。应用会限制会话内保留数量，并在启动时清理超过七天的内容。
...
应用只在本地应用数据中保存最近工作区路径和自定义 TeX 路径。工作区始终是由你管理的普通目录；“从列表移除”只清理最近记录，应用不会删除目录或其中的 `bank.json`。它不提供云同步。如果你通过其他工具同步工作区，请避免在多台设备上同时编辑同一份题库。
