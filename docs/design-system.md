...
- Desktop sidebar question rows are virtualized with dynamic heights; keyboard navigation and export selection follow the complete logical list. Sticky chapter headings retain their push-away behaviour. Narrow layouts at 760px and below keep the ordinary list.
...
- Review state is never encoded by color alone. Patterns, corner marks, numeric overflow, accessible names, preview text, and the visible legend carry the same meaning. Single-background cell text automatically chooses dark ink or white for contrast. In Error mode with multiple stripes, the question number uses an opaque paper background and dark ink so contrasting adjacent stripes cannot hide it.
