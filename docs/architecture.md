...
- `src/hooks/useBeforeCloseFlush.ts` owns the close boundary and runs two steps: commit the draft on the focused element, then flush the save queue. Fields that keep a local draft and only commit on blur (source number, chapter name, review option name/colour, history name, half-typed tags) are invisible to `flushPendingChanges` until that blur happens, so quitting mid-typing would drop them silently. The blur must run inside `flushSync` — React batches the resulting state update otherwise, and the flush would still close over the previous bank. A draft rejected by validation aborts the close and surfaces its message through the main-process unsaved dialog.
