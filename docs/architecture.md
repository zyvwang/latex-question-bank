...
- `useSaveConflictActions.ts` owns conflict dialog state, disk reload/overwrite, and single-flight save-as. It exposes a close-time wait boundary without exposing its internal promise or busy refs.
- `usePendingChanges.ts` owns draft-rejection tracking and the close boundary: wait for save-as, reject pending uploads or workspace transitions, then flush the latest bank and settings. Upload completion synchronously commits asset references before clearing the pending marker. Normal workspace changes reject pending uploads, serialize from directory selection through snapshot application, and block new uploads until the new session has been applied. A rejected close uses the existing unsaved dialog; users can return and retry after uploading, or explicitly discard.
...
- `src/hooks/useBeforeCloseFlush.ts` registers the Electron close callback and runs two steps: commit the draft on the focused element, then flush both the bank autosave queue and the workspace-settings queue. Fields that keep a local draft and only commit on blur (source number, chapter name, review option name/colour, history name, half-typed tags, and the latexmk override) are invisible to their persistence queue until that blur happens, so quitting mid-typing would drop them silently. The blur must run inside `flushSync` — React batches bank state updates otherwise, and the flush would still close over the previous bank. The latexmk override uses its own serialized latest-value queue; it is combined into this one ordered close callback rather than a separate concurrent preload listener. A draft rejected by validation or either save queue aborts the close and surfaces its message through the main-process unsaved dialog.
- Conflict save-as is single-flight from directory selection through snapshot application. Its modal cannot be dismissed and the editor/navigation subtree is inert while it runs. Pending uploads must finish before save-as starts; the request reads the latest committed bank. Closing waits for save-as and then flushes the resulting session. Cancel/failure leaves the local conflict draft intact.
- Initial AppInfo failures have a dedicated retry state without workspace recovery controls. Initial loading and retry share error handling, and only a failed bank read after successful AppInfo loading enumerates recovery candidates.
...
- Uploads carry a required `workspacePath`. The document route rejects a target mismatch before writing, then passes one captured asset directory to storage. The renderer ignores both success and failure from stale save-session generations, including A → B → A transitions.
